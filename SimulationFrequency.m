...
classdef SimulationFrequency < handle
    properties
        room
        source
        microphone
    ...
    methods(Access='private')
        function initialise(this, M)
            ...
            for i=1:M
                Gamma_S(i) = 1;
                Delta_S(i) = z.^(-this.sPropLines{i}.delaySamples);
                G_S(i) = this.sPropLines{i}.attenuation;
                Gamma_M(i) = 1;
                Delta_M(i) = z.^(-this.mPropLines{i}.delaySamples);
                G_M(i) = this.mPropLines{i}.attenuation;
            end

            
            UCell = repmat({ones(M-1,1)}, 1, M);
            U = blkdiag(UCell{:});
